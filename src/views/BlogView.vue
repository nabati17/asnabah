        ...
        <div class="flex flex-col gap-4 md:px-20 fade-zoom-up">
          <article v-for="article in articles" :key="article.id">
            <router-link :to="`/read/${article.slug}/${article.id}`" class="flex w-full bg-[#1e1e1f] border-[#383838] rounded-xl text-left text-white p-5 md:py-7 md:px-8 cursor-pointer hover:bg-[#282828] items-center">
              ...
              <div>
                <div class="w-20 h-20 md:w-28 flex items-center md:h-28 overflow-hidden rounded-lg md:rounded-xl">
                  <img :src="article.image2" class="rounded-lg md:rounded-xl" alt="">
                </div>
              </div>
            </router-link>
          </article>
...
export default {
  ...
  methods: {
    async getArticles() {
      axios.get('https://65fbe57a14650eb2100af361.mockapi.io/api/blog/all/blog/')
        .then(response => {
